...
with col1:
    ...
    source_option = st.radio("Select Input Source", ["Webcam", "Video File"])
    
    run_processing = False
    cap = None
    
    if source_option == "Webcam":
        run_processing = st.checkbox("Start Webcam", value=False)
        if run_processing:
    ...
    record_session = st.checkbox("Record Output to 'output.mp4'")
...
# Processing Loop
if run_processing and cap is not None:
    if not cap.isOpened():
        ...
        if record_session:
            ...
            out_writer = cv2.VideoWriter("output.mp4", cv2.VideoWriter_fourcc(*'mp4v'), fps, (width, height))
        ...
        if out_writer:
            out_writer.release()
            st.success("Recording saved to 'output.mp4'")
